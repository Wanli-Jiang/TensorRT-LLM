...
"""Audit the aggregate no-MTP chunking/token-budget C2048 screen."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--submissions",
        type=Path,
        default=EXPERIMENT / "submissions" / "nomtp-max-screen.tsv",
    )
    parser.add_argument(
        "--manifest",
        type=Path,
        default=EXPERIMENT / "recipes" / "nomtp-max-screen" / "manifest.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=EXPERIMENT / "reports" / "nomtp-aggregate-max-screen.json",
    )
...
def _read_submissions(path: Path) -> dict[str, dict[str, str]]:
    ...
    result = {
        row["arm"].removeprefix("nomtp-max-screen-").removesuffix("-v1"): row
        for row in rows
    }
    if tuple(result) != VARIANTS or len(rows) != len(VARIANTS):
        raise ValueError("submission ledger differs from the frozen three-arm screen")
    return result
...
def _audit_arm(
    ...
    if not serving["moe_config"].get("load_balancer"):
        raise ValueError(f"{variant}: Static528 placement is absent")
    if "speculative_config" in serving:
        raise ValueError(f"{variant}: no-MTP arm contains speculative_config")
    ...
    point = _audit_result(
        ...
        / "results_concurrency_2048_gpus_16.json",
        2048,
        3,
        16,
    )
...
def main() -> None:
    ...
    if tuple(manifest) != VARIANTS:
        raise ValueError("manifest differs from the frozen three-arm screen")
    arms = [
