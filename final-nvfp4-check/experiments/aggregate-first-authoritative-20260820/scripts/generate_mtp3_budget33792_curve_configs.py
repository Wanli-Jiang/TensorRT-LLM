...
"""Generate the MTP3 Static528 budget33792 curve and next-budget bracket."""
...
BASE = EXPERIMENT / "recipes" / "aggregate-ht-mtp3-adp16-cutedsl-static528.yaml"
VARIANTS: dict[str, dict[str, Any]] = {
    "static528-budget33792-curve": {
        "max_num_tokens": 33792,
        "moe_max_num_tokens": 33792,
        "concurrencies": [384, 512, 640, 768, 896, 1024],
        "role": "independent record confirmation and saturation curve",
    },
    "static528-budget67584-c640": {
        "max_num_tokens": 67584,
        "moe_max_num_tokens": 67584,
        "concurrencies": [640],
        "role": "focused eight-full-prefill upper-budget bracket",
    },
}
...
def _render(variant: str, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    config["name"] = f"nvfp4-current-agg-ht-mtp3-record-{variant}"
    ...
    cache_root = f"/tmp/nvfp4-mtp3-record-{variant}/{{node}}"
...
def main() -> None:
    output_dir = EXPERIMENT / "recipes" / "mtp3-budget33792-curve"
