...
"""Generate the focused MTP3 Static528 confirmation and interaction configs."""
...
VARIANTS: dict[str, dict[str, Any]] = {
    ...
    "static528-budget33792-c640": {
        ...
        "concurrencies": [640],
        "role": "focused placement-by-token-budget interaction check",
    },
}
...
def _render(variant: str, spec: dict[str, Any]) -> dict[str, Any]:
    config = copy.deepcopy(yaml.safe_load(STATIC_BASE.read_text(encoding="utf-8")))
    config["name"] = f"nvfp4-current-agg-ht-mtp3-confirm-{variant}"
    ...
    cache_root = f"/tmp/nvfp4-mtp3-static-confirm-{variant}/{{node}}"
...
def main() -> None:
    output_dir = EXPERIMENT / "recipes" / "mtp3-static-confirmation"
        ...
        manifest.append(
            {
                ...
                "path": str(path),
                "sha256": _sha256(path),
                "base": str(STATIC_BASE),
                "base_sha256": _sha256(STATIC_BASE),
                "placement": "Static528",
