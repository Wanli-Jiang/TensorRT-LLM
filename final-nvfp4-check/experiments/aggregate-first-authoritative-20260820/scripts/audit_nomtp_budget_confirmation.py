...
"""Audit the focused no-MTP budget16896 curve and budget33792 check."""
...
SCREEN_REPORT = EXPERIMENT / "reports" / "nomtp-aggregate-max-screen.json"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--submissions",
        type=Path,
        default=EXPERIMENT / "submissions" / "nomtp-budget-confirmation.tsv",
    )
    parser.add_argument(
        "--manifest",
        type=Path,
        default=(
            EXPERIMENT / "recipes" / "nomtp-budget-confirmation" / "manifest.json"
        ),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=EXPERIMENT / "reports" / "nomtp-budget-confirmation.json",
    )
...
def _audit_arm(
    ...
    if "speculative_config" in serving:
        raise ValueError(f"{variant}: no-MTP config contains speculative decoding")
    for key in (
        "return_perf_metrics",
...
def main() -> None:
    ...
    screen_winner = screen["peak"]["point"]
    curve_c2048 = next(
        point for point in arms[0]["points"] if point["concurrency"] == 2048
    )
    interaction_c2048 = arms[1]["points"][0]
    all_points = [point for arm in arms for point in arm["points"]]
    peak = max(all_points, key=lambda point: point["total_tps_per_gpu"])
    report = {
        "integrity_status": "pass",
        "screen_budget16896_c2048_tps_per_gpu": screen_winner[
            "total_tps_per_gpu"
        ],
        "repeat_budget16896_c2048_tps_per_gpu": curve_c2048[
            "total_tps_per_gpu"
        ],
        "repeat_change_percent": _change(
            curve_c2048["total_tps_per_gpu"], screen_winner["total_tps_per_gpu"]
        ),
        "budget33792_change_vs_repeat_budget16896_percent": _change(
            interaction_c2048["total_tps_per_gpu"],
            curve_c2048["total_tps_per_gpu"],
        ),
        "peak": peak,
        "arms": arms,
    }
