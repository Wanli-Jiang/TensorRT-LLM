...
readonly ATTEMPT=client-oversample-retry2
readonly LEDGER=${EXP}/submissions/disaggregate-${ATTEMPT}.tsv
...
readonly CONFIG=${EXP}/recipes/disaggregate/best-noeplb-${mode}-${ATTEMPT}.yaml
readonly OUTPUT=${EXP}/outputs/disaggregate/${mode}-best-noeplb-${ATTEMPT}
readonly SUBMIT_LOG=${EXP}/submissions/disaggregate-${mode}-${ATTEMPT}.submit.log
...
if [[ -s "${LEDGER}" ]] && awk -F '\t' -v mode="${mode}" \
    'NR > 1 && $1 == mode {found=1} END {exit !found}' "${LEDGER}"; then
    echo "A ${mode} retry is already recorded in ${LEDGER}" >&2
    exit 4
fi
