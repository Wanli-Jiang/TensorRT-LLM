...
class Qwen3NextGate(nn.Module):
    ...
    def forward_with_input_quant(
        ...
        """Return router logits and reusable FP8 block-scale input if available."""
        if self.out_dtype == hidden_states.dtype:
            result = apply_low_m_gemm_with_input_quant(self, hidden_states,
                                                       self.weight, None)
    ...
    def forward(
        ...
        def _compute_routed_output():
            supports_prequantized_input = getattr(
                self.experts, "supports_prequantized_fp8_block_scale_input",
                None)
