...
def test_planning_runner_scopes_codex_oauth_and_redacts_refreshed_tokens(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capfd: pytest.CaptureFixture[str],
) -> None:
    original_access = "planner-initial-access-token-never-persist"
    refreshed_access = "planner-refreshed-access-token-never-persist"
    refreshed_refresh = "planner-refreshed-refresh-token-never-persist"
    ...
    class RefreshingLayer:
        def __init__(self, _config: object) -> None:
            pass

        def __enter__(self) -> RefreshingLayer:
            return self
        ...
        def __call__(self, _prompt: str) -> str:
            nonlocal observed_home, invocations
            invocations += 1
            ...
            os.write(1, f"planning stdout {refreshed_access}\n".encode())
            os.write(2, f"planning stderr {refreshed_refresh}\n".encode())
            return '{"schema_version":1,"outcome":"ok"}'

    monkeypatch.setattr("agent_flow.AgentLayer", RefreshingLayer)
    result = execute_role(spec)
    captured = capfd.readouterr()

    assert result.response == '{"schema_version":1,"outcome":"ok"}'
    ...
    assert "planning stdout <redacted>" in captured.out
    assert "planning stderr <redacted>" in captured.err
    durable = Path(spec.result_path).read_text(encoding="utf-8")
