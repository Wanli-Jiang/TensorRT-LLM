...
def test_codex_oauth_is_mutually_exclusive_and_requires_bounded_strict_json(
    tmp_path: Path,
) -> None:
    with pytest.raises(CredentialError, match="select exactly one"):
        CredentialDescriptor.create(
            _binding(),
            CredentialState.BUNDLE,
            (CODEX_AUTH_JSON_CREDENTIAL, "OPENAI_API_KEY"),
            handle=CredentialHandle("test-broker", "attempt-1", 2_147_483_647),
        )
    ...
    for invalid, match in (
        ('{"tokens":{},"tokens":{}}', "duplicate JSON key"),
        ('{"tokens":{"access_token":NaN}}', "non-finite"),
        (json.dumps({"padding": "x" * MAX_CODEX_AUTH_JSON_BYTES}), "size limit"),
    ):
        with pytest.raises(CredentialError, match=match):
            materialize_credential_provision(
                workspace=tmp_path.resolve(),
                descriptor=descriptor,
                credential_values={CODEX_AUTH_JSON_CREDENTIAL: invalid},
            )
    ...
    valid = json.dumps(
        {
            "auth_mode": "chatgpt",
            "tokens": {
                "access_token": "valid-access-token-never-persist",
                "refresh_token": "valid-refresh-token-never-persist",
            },
        }
    )
    provision = materialize_credential_provision(
        workspace=tmp_path.resolve(),
        descriptor=descriptor,
        credential_values={CODEX_AUTH_JSON_CREDENTIAL: valid},
    )
    assert provision.bundle_path is not None
    assert valid not in json.dumps(descriptor.to_public_dict())
    assert load_worker_credentials(
        descriptor,
        expected_binding=_binding(),
        bundle_path=provision.bundle_path,
    ) == {CODEX_AUTH_JSON_CREDENTIAL: valid}


def test_codex_oauth_uses_temporary_home_and_tracks_refreshed_tokens(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    original_access = "initial-access-token-never-persist"
    original_refresh = "initial-refresh-token-never-persist"
    refreshed_access = "rotated-access-token-never-persist"
    refreshed_refresh = "rotated-refresh-token-never-persist"
    original = json.dumps(
        {
            "auth_mode": "chatgpt",
            "tokens": {
    ...
    monkeypatch.setenv(CODEX_AUTH_JSON_CREDENTIAL, "ambient-oauth-must-not-be-used")
    monkeypatch.setenv("OPENAI_API_KEY", "ambient-openai-must-not-be-used")
    monkeypatch.setenv(CODEX_HOME_ENVIRONMENT, "/ambient/codex-home")
    runtime_home: Path | None = None

    with agent_credential_environment(
        "codex",
        {CODEX_AUTH_JSON_CREDENTIAL: original},
    ) as redaction_values:
        assert CODEX_AUTH_JSON_CREDENTIAL not in os.environ
        assert "OPENAI_API_KEY" not in os.environ
        runtime_home = Path(os.environ[CODEX_HOME_ENVIRONMENT])
        assert runtime_home != Path("/ambient/codex-home")
        ...
        assert original_access in redaction_values.values()
        assert original_refresh in redaction_values.values()
        assert "chatgpt" not in redaction_values.values()
        auth_path.write_text(refreshed, encoding="utf-8")
        auth_path.chmod(0o600)

    assert runtime_home is not None and not runtime_home.exists()
    assert os.environ[CODEX_AUTH_JSON_CREDENTIAL] == "ambient-oauth-must-not-be-used"
    assert os.environ["OPENAI_API_KEY"] == "ambient-openai-must-not-be-used"
    assert os.environ[CODEX_HOME_ENVIRONMENT] == "/ambient/codex-home"
    assert refreshed_access in redaction_values.values()
    assert refreshed_refresh in redaction_values.values()
...
def test_generation_fence_revokes_all_old_generation_handles_idempotently(
    tmp_path: Path,
) -> None:
    workspace = tmp_path.resolve()
    adopted = prepare_credential_provision(
    ...
    assert len(receipts) == 2
    assert calls == [
        (adopted.descriptor.binding, CredentialRevocationCause.GENERATION_FENCE),
        (fenced.descriptor.binding, CredentialRevocationCause.GENERATION_FENCE),
    ]
    assert adopted.bundle_path is not None and not adopted.bundle_path.exists()
