...
def test_gate_receives_private_runtime_home_and_rejects_command_collision(
    tmp_path: Path,
) -> None:
    ...
    collision_runtime = _gate_runtime(gpu=False)
    command = collision_runtime["command"]
    assert isinstance(command, dict)
    environment = command["environment"]
    assert isinstance(environment, dict)
    environment["HOME"] = "/forged-home"
    collision_input, _worktree = _publish_input(
        collision_root,
        role=Role.GATE,
        runtime=collision_runtime,
    )

    collision = worker.execute_worker(collision_input)

    assert collision.status is artifacts.WorkerResultStatus.FAILED
    assert "environment collision" in collision.summary


def test_gate_revalidates_native_bundle_and_records_manifest_digest(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _gate_runtime(gpu=False)
    input_path, worktree = _publish_input(tmp_path, role=Role.GATE, runtime=runtime)
    package = worktree / "tensorrt_llm"
    ...
    manifest_path.write_text(
        json.dumps(
            {
                "native_build_identity": "build-id",
                "audited_base_commit": "a" * 40,
    ...
    monkeypatch.setattr(worker, "NATIVE_ARTIFACT_MANIFEST_PATH", manifest_path)
    persisted, _digest = artifacts.load_input_manifest(input_path)
    persisted_runtime = persisted.payload["runtime"]
    assert isinstance(persisted_runtime, dict)
    persisted_runtime["native_artifact_bundle"] = {
    ...
    input_path.unlink()
    artifacts.write_input_manifest(input_path, persisted)

    result = worker.execute_worker(input_path)

    assert result.status is artifacts.WorkerResultStatus.SUCCEEDED
    record = json.loads((input_path.parent / "output" / result.evidence[0].path).read_text())
    assert record["native_artifact_manifest_sha256"] == manifest_digest
...
def test_codex_oauth_worker_rotates_in_temporary_home_and_redacts_new_tokens(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capfd: pytest.CaptureFixture[str],
) -> None:
    original_access = "worker-initial-access-token-never-persist"
    refreshed_access = "worker-refreshed-access-token-never-persist"
    refreshed_refresh = "worker-refreshed-refresh-token-never-persist"
    original = json.dumps({"tokens": {"access_token": original_access}})
    ...
    binding = CredentialBinding("run-1", "item-1", "attempt-1", TASK_DIGEST, 1, "codex")
    descriptor = CredentialDescriptor.create(
        binding,
        CredentialState.BUNDLE,
        ("CODEX_AUTH_JSON",),
    ...
    monkeypatch.setattr(
        worker,
        "load_worker_credentials",
        lambda *_args, **_kwargs: {"CODEX_AUTH_JSON": original},
    )
    ...
    def invoke(**_kwargs: object) -> str:
        nonlocal invocations, observed_home
        invocations += 1
        assert "CODEX_AUTH_JSON" not in os.environ
        assert "OPENAI_API_KEY" not in os.environ
        observed_home = Path(os.environ["CODEX_HOME"])
        auth_path = observed_home / "auth.json"
        assert auth_path.read_text(encoding="utf-8") == original
        auth_path.write_text(refreshed, encoding="utf-8")
        auth_path.chmod(0o600)
        os.write(1, f"oauth stdout {refreshed_access}\n".encode())
        os.write(2, f"oauth stderr {refreshed_refresh}\n".encode())
        return _agent_response(evidence_paths=[])
    ...
    assert os.environ["CODEX_HOME"] == "/ambient/codex-home"
    assert refreshed_access not in captured.out
    assert refreshed_refresh not in captured.err
    assert "oauth stdout <redacted>" in captured.out
    assert "oauth stderr <redacted>" in captured.err
    durable = (input_path.parent / "output" / artifacts.RESULT_FILENAME).read_text(encoding="utf-8")
    assert original not in durable
    assert original_access not in durable
    assert refreshed_access not in durable
    assert refreshed_refresh not in durable
