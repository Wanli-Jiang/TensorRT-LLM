...
def test_generation_credentials_are_fenced_only_after_exact_adoption_receipts(
    ...
    assert calls == [provisions[1].descriptor.binding]
    assert len(receipts) == 1
    assert receipts[0].receipt_path.is_file()
    assert provisions[0].bundle_path is not None and provisions[0].bundle_path.is_file()
