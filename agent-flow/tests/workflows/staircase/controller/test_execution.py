...
def test_gate_materialization_validates_and_mounts_native_bundle(tmp_path: Path) -> None:
    adapter, _broker, action, _state = _adapter_fixture(tmp_path)
    package = adapter._task.repository.root / "tensorrt_llm"
    ...
    adapter._task.repository.base_commit = "a" * 40
    adapter._task.certification = SimpleNamespace(
        expected_product_identity=None,
        native_artifact_bundle=SimpleNamespace(
    ...
    manifest = replace(
        ...
        role=Role.GATE,
        profile=None,
        payload={"runtime": runtime, "context": action.manifest.payload["context"]},
    )
    gate_action = replace(action, attempt=attempt, manifest=manifest)

    materialized = adapter.materialize(gate_action)

    persisted, _digest = load_input_manifest(gate_action.attempt_dir / INPUT_FILENAME)
    persisted_runtime = persisted.payload["runtime"]
    assert isinstance(persisted_runtime, dict)
    assert persisted_runtime["native_artifact_bundle"]["manifest_sha256"] == manifest_sha256
    targets = {mount.target: mount for mount in materialized.isolation.mounts}
    candidate_package = Path("/mnt/test/workspace/candidates/item-a/item-a.0001.role/tensorrt_llm")
    assert targets[candidate_package / "libs"].source == libraries
    assert targets[candidate_package / f"bindings{ext_suffix}"].source == extension
...
def test_execution_injects_once_before_authoritative_submission_intent(tmp_path: Path) -> None:
    ...
    assert prepared.resources.partition == "cpu"
    assert prepared.resources.qos == "cpu-short"
