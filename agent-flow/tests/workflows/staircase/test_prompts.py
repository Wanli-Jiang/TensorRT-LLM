...
def test_every_role_carries_control_data_and_reference_boundaries() -> None:
    for role, raw_prompt in _prompts(STAIRCASE_PROMPTS).items():
        ...
        assert "self-containment contracts remain authoritative" in prompt, role
...
def test_assembler_profile_enforces_target_self_containment() -> None:
    ...
    assert "Keep the target self-contained" in coder
    assert "workload/tuning knobs out of routing" in coder
    assert "Reject direct tensor computation outside the catalog" in reviewer
    assert "sibling/built-in target imports" in reviewer
