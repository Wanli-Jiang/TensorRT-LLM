...
def _resource_class(*, nodes: int = 1, tasks: int = 1, gpus: int = 0) -> dict[str, object]:
    return {
        "nodes": nodes,
...
def test_packaged_examples_declare_only_non_secret_agent_identity_fields(
    ...
    assert value["execution"]["slurm"]["controller"]["credential_broker_socket"] == (
        "/shared/staircase/credential-broker.sock"
    )
