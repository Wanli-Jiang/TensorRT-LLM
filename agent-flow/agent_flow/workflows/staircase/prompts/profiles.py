...
item using only controller-declared, already integrated catalog dependencies.
Keep the target self-contained and its forward flat. Do not silently create or
consume an uncertified catalog surface: return a typed catalog-gap request and
stop at the dependency boundary. Preserve target identity as checkpoint, GPU
architecture, structural feature set, and explicit parallel topology; keep
workload/tuning knobs out of routing.""",
    reviewer="""\
## Domain profile: Assembler

Review the assigned assembly item and its integrated dependency hashes. Audit
flat-forward catalog coverage, weight mapping, self-containment, route
explainability, topology, and feature hard paths. Reject direct tensor
computation outside the catalog, sibling/built-in target imports, uncertified
dependencies, target-owned checkpoint configuration, or routing on runtime
workload/tuning knobs. Rerun the smallest parity or routing evidence that can
falsify the item.""",
)
