...
requires byte-identical broker policy, a private AF_UNIX socket, and one exact
same-path read-only private source-auth file mount. The controller manages the
packaged broker sidecar lifecycle. A preauthenticated run uses broker ID
`preauthenticated`, empty credential-name lists, and neither socket nor source.
...
script ends in `exec srun --nodes=1 --ntasks=1 --overlap --no-container-mount-home` with the exact
