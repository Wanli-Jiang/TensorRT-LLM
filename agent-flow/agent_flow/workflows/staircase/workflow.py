...
LEGACY_AGENT_TEAM_STATE_FILENAME = ".agent_team_state.json"
...
def start_run(
    *,
    ...
    legacy_state_path = workspace / LEGACY_AGENT_TEAM_STATE_FILENAME
    if legacy_state_path.exists() or legacy_state_path.is_symlink():
        raise WorkflowError(
            "legacy AgentTeam workspace cannot be resumed in place; choose a new "
            "Staircase workspace and import evidence explicitly"
        )
    ...
    if (
        ...
        with _held_controller_lease_lock(lease_path) as lock_acquired:
            ...
            if (
                ...
                raise WorkflowError(
                    "authoritative controller identity changed before recovery submission"
                )
            if (workspace / CANCEL_REQUEST_FILENAME).is_file():
                raise WorkflowError("dead-controller recovery is fenced by a cancellation request")
    ...
    *,
    ...
    if receipt_path.exists():
        ...
        if (
            ...
            or not isinstance(existing.get("successor_actions"), list)
    ...
    successor_actions, cancelled_successor = _fence_unbootstrapped_successors(
        workspace=workspace,
        state=state,
        scheduler=active_scheduler,
    )
    if cancelled_successor:
        # Scheduler cancellation is asynchronous.  Do not revoke credentials,
        # mutate attempt state, or publish a final receipt until every exact
        # successor is terminal in accounting on a later reconciliation pass.
        return
...
def _fence_unbootstrapped_successors(
    *,
    workspace: Path,
    state: RunState,
    scheduler: Scheduler,
) -> tuple[list[dict[str, object]], bool]:
    """Cancel exact unfenced successors before emergency child cleanup."""
    if state.controller_job is None:
        raise WorkflowError("successor reconciliation requires an exact predecessor job")
    predecessor = _scheduler_job(state.controller_job)
    successor_generation = state.generation + 1
    candidates: dict[str, JobIdentity | None] = {}

    pending = state.controller_successors[-1] if state.controller_successors else None
    if (
        pending is not None
        and not pending.fenced
        and pending.predecessor_generation == state.generation
    ):
        candidates[pending.submission_token] = (
            _scheduler_job(pending.successor_job) if pending.successor_job is not None else None
        )

    envelopes = [
        *sorted((workspace / LAUNCHER_DIRECTORY).glob("recovery-g*.intent.json")),
        *sorted((workspace / HUMAN_REQUESTS_DIRECTORY).glob("*.resume.intent.json")),
    ]
    if len(envelopes) > _MAX_STATUS_LAUNCHERS:
        raise WorkflowError("too many successor launcher envelopes for safe cancellation")
    for intent_path in envelopes:
        intent = _load_json_mapping(intent_path, "successor launcher intent")
        if intent.get("generation") != successor_generation:
            continue
        if (
            intent.get("schema_version") != 1
            or intent.get("kind") not in {"unexpected_recovery", "human_response"}
            or intent.get("run_id") != state.run_id
            or intent.get("task_digest") != state.task_digest
            or intent.get("predecessor_generation") != state.generation
            or _job_from_payload(intent.get("predecessor_job"), "successor launcher predecessor")
            != predecessor
        ):
            raise WorkflowError("successor launcher intent crossed a run or generation fence")
        token = intent.get("submission_token")
        if not isinstance(token, str) or not token:
            raise WorkflowError("successor launcher intent has no valid submission token")
        receipt_path = intent_path.with_name(
            intent_path.name.replace(".intent.json", ".receipt.json")
        )
        identity = None
        if receipt_path.is_file():
            receipt = _load_json_mapping(receipt_path, "successor launcher receipt")
            if receipt.get("submission_token") != token:
                raise WorkflowError("successor launcher receipt token differs from its intent")
            identity = _job_from_payload(receipt.get("job"), "successor launcher job")
        prior = candidates.get(token)
        if prior is not None and identity is not None and prior != identity:
            raise WorkflowError("successor token is bound to conflicting exact jobs")
        candidates[token] = prior or identity

    planned: list[tuple[str, JobIdentity, JobObservation, str | None]] = []
    for token, persisted_identity in sorted(candidates.items()):
        try:
            probe = scheduler.probe_submission(token)
        except SchedulerError as error:
            raise WorkflowError(f"successor cancellation probe failed closed: {error}") from error
        if len(probe.matches) > 1 or (probe.matches and probe.identity is None):
            raise WorkflowError("successor cancellation found an ambiguous submission token")
        identity = persisted_identity or probe.identity
        if persisted_identity is not None and probe.identity not in {None, persisted_identity}:
            raise WorkflowError(
                "successor scheduler identity differs from durable launcher evidence"
            )
        if identity is None:
            if not (
                probe.trustworthy
                and probe.queue_complete
                and probe.accounting_complete
                and not probe.matches
            ):
                raise WorkflowError(
                    "successor cancellation lacks certified absence or an exact job identity"
                )
            continue
        if identity.cluster != predecessor.cluster:
            raise WorkflowError("successor cancellation crossed the predecessor cluster fence")
        ownership = scheduler.verify_ownership(identity, token)
        if not ownership.matched:
            raise WorkflowError(
                f"successor cancellation ownership could not be proven: {ownership.reason}"
            )
        successor_observation = scheduler.observe_owned(identity, token)
        planned.append((token, identity, successor_observation, ownership.username))

    cancelled = False
    actions: list[dict[str, object]] = []
    for token, identity, successor_observation, username in planned:
        if (
            successor_observation.status.terminal
            and successor_observation.source is ObservationSource.ACCOUNTING
        ):
            action = "already_terminal"
        else:
        ...
        actions.append(
            {
                "job": _job_payload(identity),
                "action": action,
                "observed_status": successor_observation.status.value,
                "observation_source": successor_observation.source.value,
                "submission_token": token,
                "ownership_user": username,
            }
        )
...
def record_human_response(
    ...
    *,
    ...
    with _held_controller_lease_lock(workspace / LEASE_FILENAME) as lock_acquired:
        if not lock_acquired:
            raise WorkflowError(
                "human successor submission is fenced by a live controller lease; retry"
            )
        locked_state = load_state(workspace / STATE_FILENAME)
        if (
            locked_state.run_id != state.run_id
            ...
            raise WorkflowError(
                "authoritative controller identity changed before human successor submission"
            )
        if (workspace / CANCEL_REQUEST_FILENAME).is_file():
            raise WorkflowError("human successor submission is fenced by a cancellation request")
        return _submit_launcher_intent(
            intent_path=intent_path,
            receipt_path=receipt_path,
            scheduler=active_scheduler,
    ...
    if plan.pending:
        ...
        if orphan_plan.action is OrphanAction.CLEAN_UP:
            _revoke_expired_orphan_credentials(
                workspace=workspace,
                state=state,
                # Job-backed attempts remain active until the generation fence
                # below revokes every old-generation credential uniformly.
                # Token-only/no-job attempts become terminal during cleanup,
                # so revoke those here before they leave the active set.
                attempt_ids={target.attempt_id for target in plan.pending if target.job is None},
