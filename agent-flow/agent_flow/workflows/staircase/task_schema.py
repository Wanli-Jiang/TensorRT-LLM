...
@dataclass(frozen=True, slots=True)
class NativeArtifactBundleConfig:
    """Hash-frozen native files mounted into generic deterministic gates."""
...
class CertificationConfig:
    ...
    native_artifact_bundle: NativeArtifactBundleConfig | None = None
...
def _optional_slurm_name(value: object, path: str, errors: list[str]) -> str | None:
    if value is None:
        return None
    return _slurm_name(value, path, errors)
...
def _private_source_auth_json_path(
    value: object,
    path: str,
    errors: list[str],
) -> Path:
    """Normalize one exact private Codex OAuth source without reading it."""
    text = _nonempty_string(value, path, errors)
    if not text:
        return Path("/")
    candidate = Path(text)
    if not candidate.is_absolute() or ".." in candidate.parts:
        errors.append(f"'{path}' must be an absolute path without traversal: {text!r}")
    if candidate.as_posix() != text:
        errors.append(f"'{path}' must be a normalized exact path: {text!r}")
    if candidate.is_symlink():
        errors.append(f"'{path}' must not be a symlink: {text!r}")
    try:
        resolved = candidate.resolve(strict=True)
        info = candidate.lstat()
    except (FileNotFoundError, RuntimeError, OSError) as error:
        errors.append(f"'{path}' does not resolve to an existing private file: {text!r} ({error})")
        return candidate.absolute()
    if resolved != candidate:
        errors.append(f"'{path}' must be canonical and symlink-free: {text!r}")
    if (
        ...
        errors.append(
            f"'{path}' must be a private mode 0400/0600 regular file owned by the current "
            "user with exactly one hard link"
        )
    return resolved
...
def _parse_native_artifact_bundle(
    value: object,
    errors: list[str],
) -> NativeArtifactBundleConfig | None:
    path = "certification.native_artifact_bundle"
    if value is None:
        return None
    data = _mapping(
        value,
        path,
        frozenset({"package_root", "manifest", "manifest_sha256"}),
        frozenset({"package_root", "manifest", "manifest_sha256"}),
        errors,
    )
    package_root = _existing_path(
        data.get("package_root"), f"{path}.package_root", "directory", errors
    )
    manifest = _existing_path(data.get("manifest"), f"{path}.manifest", "file", errors)
    manifest_sha256 = _nonempty_string(
        data.get("manifest_sha256"), f"{path}.manifest_sha256", errors
    )
    if re.fullmatch(r"[0-9a-f]{64}", manifest_sha256) is None:
        errors.append(f"'{path}.manifest_sha256' must be a lowercase SHA-256 digest")
    return NativeArtifactBundleConfig(package_root, manifest, manifest_sha256)
...
def _parse_certification(
    ...
    native_artifact_bundle = _parse_native_artifact_bundle(
        data.get("native_artifact_bundle"), errors
    )
    ...
    if mode is CertificationMode.SYNTHETIC and native_artifact_bundle is not None:
        errors.append("'certification.native_artifact_bundle' is prohibited for SYNTHETIC mode")
    return CertificationConfig(mode, identity, native_artifact_bundle)
...
def _parse_mount(value: object, index: int, errors: list[str]) -> MountConfig:
    ...
    host_path = _existing_path(data.get("host_path"), f"{path}.host_path", "either", errors)
...
def _parse_controller(data: dict[str, object], errors: list[str]) -> ControllerConfig:
    ...
    source_auth_json_path: Path | None = None
    raw_source_auth_json_path = data.get("source_auth_json_path")
    if raw_source_auth_json_path is not None:
        source_auth_json_path = _private_source_auth_json_path(
            raw_source_auth_json_path,
            "execution.slurm.controller.source_auth_json_path",
            errors,
        )
...
def _parse_resource_class(name: str, value: object, errors: list[str]) -> ResourceClass:
    ...
    return ResourceClass(
        ...
        gpu_allocation_padding=(
            _strict_bool(
                data["gpu_allocation_padding"],
                f"{path}.gpu_allocation_padding",
                errors,
            )
            if "gpu_allocation_padding" in data
            else False
        ),
        partition=_optional_slurm_name(data.get("partition"), f"{path}.partition", errors),
        qos=_optional_slurm_name(data.get("qos"), f"{path}.qos", errors),
    )
...
def _parse_smith(data: dict[str, object], target: TargetConfig, errors: list[str]) -> SmithConfig:
    ...
    for resource in resources:
        ...
        if resource.gpu_allocation_padding and resource.name != "deterministic_gate":
            errors.append(
                f"'{prefix}.gpu_allocation_padding' is only supported for deterministic_gate"
            )
    ...
    name = resource.name
    if resource.total_tasks != target.world_size:
        errors.append(
            f"'execution.slurm.smith.resource_classes.{name}' must request exactly "
            f"target.world_size={target.world_size} tasks, got "
            f"tasks={resource.total_tasks}, GPUs={resource.total_gpus}"
        )
    if resource.gpu_allocation_padding:
        if target.world_size != 1 or resource.nodes != 1 or resource.tasks_per_node != 1:
            errors.append(
                f"'execution.slurm.smith.resource_classes.{name}.gpu_allocation_padding' "
                "is only supported for a one-rank target on one node"
            )
        if resource.total_gpus <= target.world_size:
            errors.append(
                f"'execution.slurm.smith.resource_classes.{name}.gpu_allocation_padding' "
                "requires more allocated GPUs than target.world_size"
            )
    elif resource.total_gpus != target.world_size:
        errors.append(
            f"'execution.slurm.smith.resource_classes.{name}' must request exactly "
            f"target.world_size={target.world_size} tasks and GPUs unless explicit "
            "gpu_allocation_padding is enabled, got "
            f"tasks={resource.total_tasks}, GPUs={resource.total_gpus}"
        )
...
def _parse_credential_broker_policy(
    ...
    if backend_kind == "codex" and {"CODEX_AUTH_JSON", "OPENAI_API_KEY"}.issubset(names):
        errors.append(
            f"'{path}.allowed_credential_names' must select exactly one of "
            "'CODEX_AUTH_JSON' or 'OPENAI_API_KEY'"
        )
    ttl = _positive_int(
...
def _parse_role_classes(
    ...
    for role_name in _ROLE_CLASS_NAMES:
        ...
        elif resource.gpu_allocation_padding:
            errors.append(
                f"'{role_path}.resource_class' cannot bind a GPU allocation-padding resource"
            )
...
def _validate_credential_broker_deployment(
    ...
    if credentialed:
        ...
        if first.allowed_credential_names == ("CODEX_AUTH_JSON",):
            if source_auth_json_path is None:
                errors.append(
                    "'execution.slurm.controller.source_auth_json_path' is required for the "
                    "managed CODEX_AUTH_JSON broker"
                )
        elif source_auth_json_path is not None:
            errors.append(
                "'execution.slurm.controller.source_auth_json_path' is permitted only when "
                "all roles request exactly CODEX_AUTH_JSON"
            )
        return
    ...
    if source_auth_json_path is not None:
        errors.append(
            "'execution.slurm.controller.source_auth_json_path' must be omitted when all "
            "roles are preauthenticated"
        )
...
def _validate_managed_credential_broker_filesystem(
    repository: RepositoryConfig,
    execution: ExecutionConfig,
    errors: list[str],
) -> None:
    """Validate the non-secret filesystem envelope of the managed OAuth sidecar."""
    controller = execution.slurm.controller
    source = controller.source_auth_json_path
    if source is None:
        return
    ...
    workspace_socket = socket_path.is_relative_to(repository.workspace_root)
    private_runtime_socket = (
        ...
        and _SLUG_RE.fullmatch(socket_path.parent.name) is not None
        and socket_path.name == "broker.sock"
    )
    if not workspace_socket and not private_runtime_socket:
        errors.append(
            "'execution.slurm.controller.credential_broker_socket' must be beneath "
            "repository.workspace_root or use the private "
            "'/tmp/staircase-credential-broker-*/broker.sock' runtime convention"
        )
    if len(os.fsencode(socket_path)) > 103:
        errors.append(
            "'execution.slurm.controller.credential_broker_socket' exceeds the Unix-socket "
            "path limit"
        )
    exact_source_mounts = tuple(
        mount
        for mount in controller.mounts
        if mount.host_path == source and mount.container_path == source.as_posix()
    )
    if len(exact_source_mounts) != 1 or not exact_source_mounts[0].read_only:
        errors.append(
            "'execution.slurm.controller.source_auth_json_path' must have exactly one "
            "identity-mapped read-only controller mount"
        )
    covering_source_mounts = tuple(
        mount
        for mount in controller.mounts
        if source == mount.host_path or source.is_relative_to(mount.host_path)
    )
    if covering_source_mounts != exact_source_mounts:
        errors.append(
            "'execution.slurm.controller.source_auth_json_path' must not be exposed through "
            "a broader controller mount"
        )
...
def _source_task_dict(task: NormalizedTask) -> dict[str, object]:
    ...
    return {
        ...
        "certification": {
            ...
            "native_artifact_bundle": (
                {
                    "package_root": task.certification.native_artifact_bundle.package_root.as_posix(),
                    "manifest": task.certification.native_artifact_bundle.manifest.as_posix(),
                    "manifest_sha256": (task.certification.native_artifact_bundle.manifest_sha256),
                }
                if task.certification.native_artifact_bundle is not None
                else None
            ),
        },
        "execution": {
            ...
            "slurm": {
                ...
                "controller": {
                    ...
                    "source_auth_json_path": (
                        controller.source_auth_json_path.as_posix()
                        if controller.source_auth_json_path is not None
                        else None
                    ),
                },
                "agent_worker": {
                ...
                "smith": {
                    ...
                    "resource_classes": {
                        resource.name: {
                            ...
                            "gpu_allocation_padding": resource.gpu_allocation_padding,
...
def _normalize_task_value(
    ...
    *,
    ...
    certification_data = _required_mapping(
        ...
        "certification",
        "",
        frozenset({"mode", "expected_product_identity", "native_artifact_bundle"}),
        frozenset({"mode"}),
        errors,
    )
    ...
    if certification.native_artifact_bundle is not None and (
        gates.collective_tests or target.world_size != 1
    ):
        errors.append(
            "'certification.native_artifact_bundle' currently supports only generic "
            "single-process gates; target.world_size must be 1 and "
            "gates.collective_tests must be empty"
        )
    _validate_gate_classes(certification, gates, target, execution, errors)
    if (
        certification.mode is CertificationMode.LOCAL
        and execution_override != "local"
        and certification.native_artifact_bundle is None
    ):
        errors.append(
            "'certification.mode: LOCAL' requires the explicit CLI local execution override "
            "unless a hash-frozen native_artifact_bundle binds generic Slurm gates"
        )
    ...
    _validate_managed_credential_broker_filesystem(repository, execution, errors)
