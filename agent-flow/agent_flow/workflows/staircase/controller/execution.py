...
from ..common.native_artifacts import (
    ...
    def _prepared_manifest(
        ...
        *,
        ...
        if kind == "agent":
            ...
            certification = getattr(self._task, "certification", None)
            bundle_config = (
                None
                if certification is None
                else getattr(certification, "native_artifact_bundle", None)
            )
            if bundle_config is None:
                manifest = action.manifest
            else:
                receipt = runtime.get("receipt")
                if not isinstance(receipt, dict) or receipt.get("scope") not in {
                    "cpu_static",
                    "single_gpu_product",
                }:
                    raise ExecutionAdapterError(
                        "native artifact bundles support only generic single-process gates"
                    )
                native_artifact_bundle = validate_native_artifact_bundle(
                    package_root=bundle_config.package_root,
                    manifest_path=bundle_config.manifest,
                    manifest_sha256=bundle_config.manifest_sha256,
                    expected_native_build_identity=controller.build_identity,
                    expected_base_commit=self._task.repository.base_commit,
                )
                gate_runtime = dict(runtime)
                gate_runtime["native_artifact_bundle"] = native_artifact_runtime(
        ...
        isolation = build_worker_isolation(
            ...
            native_artifact_bundle=native_artifact_bundle,
        )
        if kind == "agent":
