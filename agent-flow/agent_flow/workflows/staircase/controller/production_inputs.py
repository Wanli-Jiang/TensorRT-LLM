...
class GateResourceIntent:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.gpu_allocation_padding, bool):
            raise ProductionInputError("gate resource gpu_allocation_padding must be boolean")
        for name, value in (("partition", self.partition), ("qos", self.qos)):
            if value is not None and (not isinstance(value, str) or not value):
                raise ProductionInputError(f"gate resource {name} must be a non-empty string")
...
class GateScopeIntent:
    ...
    def __post_init__(self) -> None:
        ...
        expected_gpus_per_node = expected_gpus * self.resource.tasks_per_node
        if self.resource.gpu_allocation_padding:
            if (
                self.execution
                not in {
                    GateExecutionIntent.SINGLE_GPU_PRODUCT,
                    GateExecutionIntent.TARGET_PRODUCT,
                }
                or self.expected_world_size != 1
                or self.resource.nodes != 1
                or self.resource.tasks_per_node != 1
                or self.resource.gpus_per_node <= expected_gpus_per_node
            ):
                raise ProductionInputError(
                    "gate GPU allocation padding requires one product rank "
                    "and additional allocated GPUs"
                )
        elif self.resource.gpus_per_node != expected_gpus_per_node:
...
def resolve_production_inputs(
    ...
    gate_scope_intents = tuple(
        _gate_scope_intent(
            ...
            envelope_gpu_allocation_padding=gate_envelope.gpu_allocation_padding,
            envelope_partition=gate_envelope.partition,
            envelope_qos=gate_envelope.qos,
...
def _gate_scope_intent(
    ...
    *,
    ...
    envelope_gpu_allocation_padding: bool,
    envelope_partition: str | None,
    envelope_qos: str | None,
    ...
    if phase in {GatePhase.CLAIMS_ROUTING_NO_STALE, GatePhase.NATIVE_CONTRACT}:
        ...
        world_size = 1
        if target_world_size == 1 and envelope_gpu_allocation_padding:
            topology = (1, 1, envelope_gpus_per_node)
            gpu_allocation_padding = True
        else:
            topology = (1, 1, 1)
            gpu_allocation_padding = False
        partition = envelope_partition
        qos = envelope_qos
    else:
        ...
        if (
            ...
            or (
                not envelope_gpu_allocation_padding
                and envelope_gpus_per_node != envelope_tasks_per_node
            )
            or (
                envelope_gpu_allocation_padding
                and (
                    target_world_size != 1
                    or envelope_nodes != 1
                    or envelope_tasks_per_node != 1
                    or envelope_gpus_per_node <= 1
                )
            )
        ):
            raise ProductionInputError(
        ...
        gpu_allocation_padding = envelope_gpu_allocation_padding
        partition = envelope_partition
        qos = envelope_qos
    resource = GateResourceIntent(
        "deterministic_gate",
