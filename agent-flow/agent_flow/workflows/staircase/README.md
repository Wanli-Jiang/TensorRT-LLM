...
- **Credentialed:** all seven role classes use byte-identical normalized
  ...
  configure one absolute `controller.credential_broker_socket`. The broker ID
  cannot be `preauthenticated`.

The credentialed mode is an external trust boundary. This package contains a
bounded AF_UNIX client, not a broker daemon. Before controller mutation, the
client verifies a non-symlink Unix socket with mode `0600` or `0660`, connects
without sending a protocol frame or credential name, and confirms that the
socket inode did not change. Later canonical, length-framed requests exchange
only public attempt bindings, opaque handles, paths, and receipts. Credential
values never enter the task, controller arguments, scheduler environment,
state, evidence, logs, or reports. The external broker must materialize an
owned mode-`0600` bundle below the controller-private credential root; workers
receive it at a fixed read-only mount only after admission. Terminal,
cancellation, replacement, generation-fence, and expired-orphan paths revoke
the exact handle.
...
Assembler implements one target core, feature, weights, or routing item using
already integrated catalog dependencies. A missing catalog surface blocks the
item and requires a planned Smith dependency; Assembler cannot silently create
or consume an uncertified catalog wrapper.
...
definitions, and existing ModelingV2 targets and catalog entries. Reading a
reference does not relax target self-containment: checked-in targets may use
catalog entries but cannot import model helpers or target files from the
built-in zoo or a sibling target.
...
correctness, and a high-risk expected output cannot come only from the
implementation under test or a helper that shares its bug.
