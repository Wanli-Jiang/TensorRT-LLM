...
def _validate_resources(
    ...
    for resource in smith.resource_classes:
        ...
        if resource.gpu_allocation_padding and resource.name != "deterministic_gate":
            add(
                "resource_topology",
                f"resource class {resource.name!r} enables GPU allocation padding outside "
                "deterministic_gate",
            )
        if resource.name not in {"deterministic_gate", "reviewer_rerun"}:
            continue
        tasks_match = resource.total_tasks == task.target.world_size
        if resource.gpu_allocation_padding:
            gpus_match = (
                resource.name == "deterministic_gate"
                and task.target.world_size == 1
                and resource.nodes == 1
                and resource.tasks_per_node == 1
                and resource.total_gpus > task.target.world_size
            )
        else:
            gpus_match = resource.total_gpus == task.target.world_size
        if not tasks_match or not gpus_match:
            add(
                "resource_topology",
                f"resource class {resource.name!r} does not request target.world_size="
                f"{task.target.world_size} ranks with exact GPUs or admitted single-rank "
                "GPU allocation padding",
            )
