...
MAX_CODEX_AUTH_JSON_BYTES = 24_576
MAX_CODEX_AUTH_JSON_DEPTH = 16
MAX_CODEX_AUTH_JSON_NODES = 1_024
MAX_CODEX_AUTH_JSON_KEY_BYTES = 256
MAX_CODEX_AUTH_JSON_STRING_BYTES = 32_768
CODEX_AUTH_JSON_CREDENTIAL = "CODEX_AUTH_JSON"
...
_CODEX_SECRET_LEAF = re.compile(
    r"(?:^|_)(?:api_?key|cookie|password|secret|token)(?:$|_)",
    re.IGNORECASE,
)
...
BACKEND_CREDENTIAL_ALLOWLIST: Mapping[BackendKind, frozenset[str]] = {
    ...
    "codex": frozenset({CODEX_AUTH_JSON_CREDENTIAL, "OPENAI_API_KEY"}),
}
...
class CredentialDescriptor:
    ...
    def __post_init__(self) -> None:
        ...
        _validate_credential_name_combination(self.binding.backend_kind, self.credential_names)
...
def materialize_credential_provision(
    *,
    ...
    credentials = _validated_credentials(descriptor.binding.backend_kind, credential_values)
...
def revoke_generation_fenced_provisions(
    *,
    ...
    """Revoke every exact old-generation provision after reconciliation.
    ...
    job adoptions. Job adoption proves scheduler ownership but never adopts an
    old-generation credential: every supplied descriptor is revoked so a
    restarted controller cannot reuse predecessor authority. All inputs are
    validated before the first broker mutation; the controller never resolves
    the private bundle behind a descriptor.
    """
    if (
...
def load_worker_credentials(
    ...
    *,
    ...
    return _validated_credentials(descriptor.binding.backend_kind, credential_data)
...
def agent_credential_environment(
    ...
    """Expose one bundle to an agent without exporting Codex OAuth JSON.

    ``CODEX_AUTH_JSON`` is parsed and written to a private, temporary
    ``CODEX_HOME/auth.json``.  The file remains writable so Codex can rotate
    tokens, and it is validated and removed as soon as the invocation exits.
    The yielded mapping contains only in-memory values used to redact the
    original and refreshed token leaves.
    """
    redaction_values = _credential_redaction_values(
        backend_kind,
        _validated_credentials(backend_kind, credentials),
    )
    with _agent_credential_environment_with_redactions(
        backend_kind,
        credentials,
        redaction_values,
    ):
        yield redaction_values


@contextmanager
def _agent_credential_environment_with_redactions(
    backend_kind: BackendKind,
    credentials: Mapping[str, str],
    redaction_values: MutableMapping[str, str],
) -> Iterator[None]:
    """Implement the environment/file scope with a caller-owned redaction set."""
    checked = _validated_credentials(backend_kind, credentials)
    absent = object()
    previous: dict[str, object] = {
        name: os.environ.get(name, absent) for name in ALL_BACKEND_CREDENTIAL_NAMES
    }
    previous_codex_home = os.environ.get(CODEX_HOME_ENVIRONMENT, absent)
    codex_auth_json = checked.get(CODEX_AUTH_JSON_CREDENTIAL)
    environment_credentials = {
        name: value for name, value in checked.items() if name != CODEX_AUTH_JSON_CREDENTIAL
    }
    try:
        for name in ALL_BACKEND_CREDENTIAL_NAMES:
            os.environ.pop(name, None)
        os.environ.update(environment_credentials)
        if backend_kind == "codex" and checked:
            with tempfile.TemporaryDirectory(prefix="staircase-codex-home-") as temporary:
                codex_home = Path(temporary)
                codex_home.chmod(0o700)
                os.environ[CODEX_HOME_ENVIRONMENT] = str(codex_home)
                auth_path = codex_home / CODEX_AUTH_FILENAME
                if codex_auth_json is not None:
                    _write_codex_auth_json(auth_path, codex_auth_json)
                try:
                    yield
                finally:
                    if codex_auth_json is not None:
                        refreshed = _read_codex_auth_json(auth_path, "refreshed Codex auth JSON")
                        _add_codex_auth_redactions(
                            redaction_values,
                            refreshed,
                            label="refreshed",
                        )
        ...
        if previous_codex_home is absent:
            os.environ.pop(CODEX_HOME_ENVIRONMENT, None)
        else:
            os.environ[CODEX_HOME_ENVIRONMENT] = cast(str, previous_codex_home)


@contextmanager
def agent_credential_runtime(
    backend_kind: BackendKind,
    credentials: Mapping[str, str],
) -> Iterator[dict[str, str]]:
    """Scope credentials and redact durable streams for one agent call."""
    redaction_values = _credential_redaction_values(
        backend_kind,
        _validated_credentials(backend_kind, credentials),
    )
    with redact_durable_standard_streams(redaction_values):
        with _agent_credential_environment_with_redactions(
            backend_kind,
            credentials,
            redaction_values,
    ...
    try:
        with tempfile.TemporaryFile() as stdout, tempfile.TemporaryFile() as stderr:
            ...
            try:
                ...
                os.dup2(saved[0], 1)
                os.dup2(saved[1], 2)
                _replay_redacted_stream(stdout, saved[0], credentials)
                _replay_redacted_stream(stderr, saved[1], credentials)
    finally:
        os.close(saved[0])
        os.close(saved[1])
...
def _select_credentials(
    ...
    return _validated_credentials(backend_kind, selected)
...
def _write_exclusive_bundle(path: Path, payload: Mapping[str, object]) -> None:
    ...
    if len(serialized) > MAX_CREDENTIAL_BUNDLE_BYTES:
        raise CredentialError("credential bundle exceeds the size limit")
    flags = os.O_WRONLY | os.O_CREAT | os.O_EXCL
    if hasattr(os, "O_NOFOLLOW"):
        flags |= os.O_NOFOLLOW
...
def _validate_credential_name_combination(
    backend_kind: BackendKind,
    credential_names: Sequence[str],
) -> None:
    names = set(credential_names)
    if backend_kind == "codex" and {
        CODEX_AUTH_JSON_CREDENTIAL,
        "OPENAI_API_KEY",
    }.issubset(names):
        raise CredentialError(
            "Codex credentials must select exactly one of CODEX_AUTH_JSON or OPENAI_API_KEY"
        )
...
def _validated_credentials(
    ...
    _validate_credential_name_combination(backend_kind, tuple(credentials))
    checked = {name: _credential_value(value, name) for name, value in credentials.items()}
    codex_auth_json = checked.get(CODEX_AUTH_JSON_CREDENTIAL)
    if codex_auth_json is not None:
        _parse_codex_auth_json(codex_auth_json, "Codex auth JSON")
    return checked


def _parse_codex_auth_json(raw: str, label: str) -> dict[str, object]:
    try:
        encoded = raw.encode("utf-8")
    except UnicodeEncodeError as error:
        raise CredentialError(f"{label} is not strict UTF-8 JSON") from error
    if len(encoded) > MAX_CODEX_AUTH_JSON_BYTES:
        raise CredentialError(f"{label} exceeds the size limit")

    def reject_constant(value: str) -> object:
        raise CredentialError(f"{label} contains non-finite number {value!r}")
    ...
    def validate_node(node: object, depth: int) -> None:
        nonlocal node_count
        node_count += 1
        if node_count > MAX_CODEX_AUTH_JSON_NODES:
            raise CredentialError(f"{label} exceeds the node limit")
        if depth > MAX_CODEX_AUTH_JSON_DEPTH:
            raise CredentialError(f"{label} exceeds the nesting limit")
        if isinstance(node, dict):
            for key, child in node.items():
                if len(key.encode("utf-8")) > MAX_CODEX_AUTH_JSON_KEY_BYTES:
                    raise CredentialError(f"{label} contains an oversized key")
        ...
        if isinstance(node, str):
            if len(node.encode("utf-8")) > MAX_CODEX_AUTH_JSON_STRING_BYTES:
                raise CredentialError(f"{label} contains an oversized string")
            return
        if isinstance(node, float) and not math.isfinite(node):
            raise CredentialError(f"{label} contains a non-finite number")
        if node is None or isinstance(node, (bool, int, float)):
            return
        raise CredentialError(f"{label} contains an unsupported JSON value")

    validate_node(data, 0)
    return data


def _codex_auth_secret_leaves(value: object) -> tuple[str, ...]:
    secrets: list[str] = []
    ...
    def visit(node: object, key: str | None = None) -> None:
        ...
        if (
            ...
            and node
            and key is not None
            and _CODEX_SECRET_LEAF.search(key) is not None
        ):
            secrets.append(node)
...
def _credential_redaction_values(
    ...
    codex_auth_json = values.get(CODEX_AUTH_JSON_CREDENTIAL)
    if backend_kind == "codex" and codex_auth_json is not None:
        _add_codex_auth_redactions(values, codex_auth_json, label="initial")
    return values


def _add_codex_auth_redactions(
    redaction_values: MutableMapping[str, str],
    raw: str,
    *,
    label: str,
) -> None:
    parsed = _parse_codex_auth_json(raw, f"{label} Codex auth JSON")
    if raw not in redaction_values.values():
        redaction_values[f"CODEX_AUTH_JSON_{label.upper()}_DOCUMENT"] = raw
    for index, secret in enumerate(_codex_auth_secret_leaves(parsed)):
        if secret in redaction_values.values():
            continue
        redaction_values[f"CODEX_AUTH_JSON_{label.upper()}_SECRET_{index:04d}"] = secret


def _write_codex_auth_json(path: Path, raw: str) -> None:
    _parse_codex_auth_json(raw, "Codex auth JSON")
    flags = os.O_WRONLY | os.O_CREAT | os.O_EXCL
    if hasattr(os, "O_NOFOLLOW"):
        flags |= os.O_NOFOLLOW
    descriptor = os.open(path, flags, 0o600)
...
def _read_codex_auth_json(path: Path, label: str) -> str:
    try:
        _validate_private_file(
            path,
            mode=0o600,
        ...
        raise CredentialError("Codex auth file disappeared during agent invocation") from error
    if len(raw_bytes) > MAX_CODEX_AUTH_JSON_BYTES:
        raise CredentialError(f"{label} exceeds the size limit")
    try:
        raw = raw_bytes.decode("utf-8")
    except UnicodeDecodeError as error:
        raise CredentialError(f"{label} is not strict UTF-8 JSON") from error
    _parse_codex_auth_json(raw, label)
    return raw
