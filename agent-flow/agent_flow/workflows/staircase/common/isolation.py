...
from .native_artifacts import NATIVE_ARTIFACT_MANIFEST_PATH, ValidatedNativeArtifactBundle
...
def build_worker_isolation(
    *,
    ...
    native_artifact_bundle: ValidatedNativeArtifactBundle | None = None,
) -> WorkerIsolation:
    ...
    if native_artifact_bundle is not None:
        if canonical_worktree is None or worktree_writable:
            raise IsolationPolicyError("native artifacts require an immutable candidate worktree")
        if _overlaps(native_artifact_bundle.package_root, canonical_workspace) or _overlaps(
            native_artifact_bundle.manifest_path, canonical_workspace
        ):
            raise IsolationPolicyError("native artifact bundle cannot overlap controller state")
        native_mounts = _native_artifact_mounts(
            translator,
            canonical_worktree,
            native_artifact_bundle,
        )
        existing_sources = {mount.source for mount in exact_mounts}
        existing_targets = {mount.target for mount in exact_mounts}
        if existing_sources.intersection(mount.source for mount in native_mounts):
            raise IsolationPolicyError("native artifact mount source collides with a worker mount")
        if existing_targets.intersection(mount.target for mount in native_mounts):
            raise IsolationPolicyError("native artifact mount target collides with a worker mount")
        exact_mounts = (*exact_mounts, *native_mounts)
...
def _native_artifact_mounts(
    translator: ContainerPathTranslator,
    worktree: Path,
    bundle: ValidatedNativeArtifactBundle,
) -> tuple[Mount, ...]:
    """Create nested read-only mounts over one candidate package."""
    package_root = _canonical_existing_directory(bundle.package_root, "native package root")
    manifest = _canonical_existing_path(bundle.manifest_path, "native artifact manifest")
    candidate_package = translator.to_container(worktree) / "tensorrt_llm"
    mounts = [
        Mount(package_root / "libs", candidate_package / "libs", True),
        Mount(manifest, NATIVE_ARTIFACT_MANIFEST_PATH, True),
    ]
    mounts.extend(
        Mount(
            package_root / entry.path,
            candidate_package / entry.path,
            True,
        )
        for entry in bundle.top_level_extensions
    )
    ...
    if len(sources) != len(set(sources)) or len(targets) != len(set(targets)):
        raise IsolationPolicyError("native artifact mounts must have unique sources and targets")
    for source in sources:
        translator.to_container(source)
    return tuple(mounts)
