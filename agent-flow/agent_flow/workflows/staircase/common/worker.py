...
class GateRuntimeSpec:
    ...
    native_artifact_bundle: NativeArtifactRuntimeSpec | None = None


@dataclass(frozen=True, slots=True)
class NativeArtifactRuntimeSpec:
    """Controller-validated native bundle identity rechecked by the gate."""
...
def _execute_agent(
    ...
    try:
        ...
        with agent_credential_runtime(spec.backend_kind, credentials) as protected_credentials:
...
def _execute_gate(
    ...
    try:
        ...
        native_manifest_sha256 = _validate_gate_native_artifacts(
            worktree, spec.native_artifact_bundle
        )
    ...
    try:
        ...
        runtime_environment = _gate_runtime_environment(attempt_dir)
        record = _run_gate_command(
            spec.command,
            cwd,
            (*trusted_environment, *runtime_environment),
        )
        if native_manifest_sha256 is not None:
            record["native_artifact_manifest_sha256"] = native_manifest_sha256
...
def _parse_gate_runtime(runtime: dict[str, object]) -> GateRuntimeSpec:
    ...
    if "native_artifact_bundle" in runtime:
        expected_keys.add("native_artifact_bundle")
    _exact_keys(runtime, expected_keys, "gate runtime")
    _schema_version(runtime)
    ...
    return GateRuntimeSpec(
        ...
        native_artifact_bundle=(
            _parse_native_artifact_runtime(runtime["native_artifact_bundle"])
            if "native_artifact_bundle" in runtime
            else None
        ),
    )
...
def _parse_native_artifact_runtime(value: object) -> NativeArtifactRuntimeSpec:
    data = _required_object(value, "native artifact runtime")
    _exact_keys(
        data,
        {
            "manifest_path",
    ...
    manifest_path = _required_string(data, "manifest_path")
    if manifest_path != NATIVE_ARTIFACT_MANIFEST_PATH.as_posix():
        raise WorkerRuntimeError("native artifact runtime must use the fixed manifest path")
    return NativeArtifactRuntimeSpec(
        manifest_sha256=_required_digest(data, "manifest_sha256"),
        native_build_identity=_required_string(data, "native_build_identity"),
        audited_base_commit=_required_hex(data, "audited_base_commit", 40),
        python_ext_suffix=_required_string(data, "python_ext_suffix"),
    )
...
def _validate_gate_native_artifacts(
    worktree: Path,
    runtime: NativeArtifactRuntimeSpec | None,
) -> str | None:
    if runtime is None:
        return None
    package_root = worktree / "tensorrt_llm"
    try:
        bundle = validate_native_artifact_bundle(
            package_root=package_root,
            manifest_path=NATIVE_ARTIFACT_MANIFEST_PATH,
            manifest_sha256=runtime.manifest_sha256,
            expected_native_build_identity=runtime.native_build_identity,
            expected_base_commit=runtime.audited_base_commit,
        )
    except NativeArtifactError as error:
        raise WorkerRuntimeError(f"native artifact validation failed: {error}") from error
    if bundle.manifest.python_ext_suffix != runtime.python_ext_suffix:
        raise WorkerRuntimeError("native artifact Python EXT_SUFFIX differs from runtime binding")
    spec = PathFinder.find_spec("tensorrt_llm", [str(worktree)])
    expected_origin = package_root / "__init__.py"
    if spec is None or spec.origin is None:
        raise WorkerRuntimeError("candidate TensorRT-LLM package is not importable")
    try:
        observed_origin = Path(spec.origin).resolve(strict=True)
        canonical_expected = expected_origin.resolve(strict=True)
    except OSError as error:
        raise WorkerRuntimeError("candidate TensorRT-LLM import origin is not regular") from error
    if observed_origin != canonical_expected:
        raise WorkerRuntimeError(
            "candidate TensorRT-LLM import origin differs from the immutable worktree"
        )
    return bundle.manifest.sha256
...
def _gate_runtime_environment(attempt_dir: Path) -> tuple[tuple[str, str], ...]:
    """Create private writable caches for a home-less generic gate container."""
    cache_root = attempt_dir / "runtime-cache"
    paths = {
        "HOME": attempt_dir / "runtime-home",
    ...
    for path in (cache_root, *paths.values()):
        ...
        if path.is_symlink() or not path.is_dir():
            raise WorkerRuntimeError("gate runtime cache must be a regular directory")
        if path.resolve(strict=True) != path or stat.S_IMODE(path.stat().st_mode) != 0o700:
            raise WorkerRuntimeError("gate runtime cache must be canonical with mode 0700")
    return tuple(sorted((name, str(path)) for name, path in paths.items()))
...
def _required_digest(data: Mapping[str, object], name: str) -> str:
    value = _optional_digest(data[name], name)
    if value is None:
        raise WorkerRuntimeError(f"{name} must be a lowercase SHA-256 digest")
    return value


def _required_hex(data: Mapping[str, object], name: str, length: int) -> str:
    value = _required_string(data, name)
    if len(value) != length or re.fullmatch(r"[0-9a-f]+", value) is None:
        raise WorkerRuntimeError(f"{name} must be {length} lowercase hexadecimal characters")
    return value
