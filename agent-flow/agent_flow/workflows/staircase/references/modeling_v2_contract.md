...
references. The frozen task or approved plan selects one checked-in boundary:

- **Self-contained catalog target (default):** the target is flat for one
...
synthetic class under `TRTLLM_MODELING_V2=require`.
...
For a self-contained target, the catalog is its tensor-computation vocabulary.
Every operation that creates or transforms a tensor is a catalog call; only
tensor metadata reads and Python control flow live directly in the target. A
delegated target instead owns a narrow adapter and must not silently copy,
extend, or partially fork its declared built-in dependencies.
...
a built-in implementation. A delegated target still routes to its exact
synthetic ModelingV2 class; its declared internal dependency is not top-level
fallback. The older `MODELING_V2_TARGET` and obsolete `TRTLLM_STAIRCASE` names
...
1. static routing, target-identity, and required-op checks, plus either catalog
   self-containment or the exact declared delegation boundary;
...
Boot catches construction failures; it is not accuracy. A delegated target
must also prove real published-checkpoint loading and generation. Performance
is measured evidence, never the universal release gate. Required correctness,
accuracy, the selected implementation boundary, exact no-fallback routing,
and feature-acceptance failures are hard failures regardless of score or speed.
Reviewer and QA validate the frozen candidate independently.
