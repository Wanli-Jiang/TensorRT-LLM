...
def test_regex_rename_transfers_consumable_weight_ownership():
    ...
    renamed = rename_weights_with_regex(
...
def test_pageout_safetensors_after_moe_load():
    with (
            ...
            mock.patch(
                "tensorrt_llm._torch.mmap_utils.pageout_file_backed_regions") as
            pageout_file_backed_regions,
    ):
        _pageout_safetensors_after_moe_load()

    synchronize.assert_called_once_with()
    pageout_file_backed_regions.assert_called_once_with(".safetensors",
                                                        mode="dontneed")
...
def test_get_load_weights_num_workers_rejects_invalid_values(
        monkeypatch, value):
